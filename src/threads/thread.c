...
if (thread_mlfqs)
  {
    ...
    thread_current()->virtual_priority = check_priority_bound(prio);
  }
...
thread_unblock (struct thread *t) 
{
  ...
  list_insert_ordered (&ready_list,&t->elem,list_max_comp,NULL);
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    list_insert_ordered (&ready_list,&cur->elem,list_max_comp,NULL);
...
thread_set_priority (int new_priority) 
{
  ...
  struct thread *t = thread_current ();
  int old_priority = t->virtual_priority;
...
int check_priority_bound(int priority)
{
 if(priority>PRI_MAX)
  return PRI_MAX;
 if(priority<PRI_MIN)
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  old_level = intr_disable ();
  list_insert_ordered (&all_list, &t->allelem,list_max_comp,NULL);
  intr_set_level (old_level);
}
