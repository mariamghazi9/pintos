...
static bool list_less_comp(const struct list_elem*a,const struct list_elem*b, void*aux UNUSED)
{
  ...
  return first->sleep_time < second->sleep_time;
  
 }
//list_insert_ordered(blockingThreads);
...
timer_sleep (int64_t ticks) 
{
  ...
  ASSERT (intr_get_level () == INTR_ON);
   ...
   list_insert_ordered(&blockedThreads,&thread_current ()->elem,list_less_comp,NULL);
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  iter= list_begin(&blockedThreads);
    while( iter != list_end (&blockedThreads))
    {  
